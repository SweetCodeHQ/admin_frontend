...
const Button = ({ text, handleClick }) => {
  return (
    <button
      className="text-sm self-center border rounded-xl p-2 mb-3 cursor-pointer transition delay-50 ease-in-out hover:-translate-y-1 hover:scale-110"
      onClick={handleClick}
    >
      {text}
    </button>
...
const TopicAbstractMenu = ({ topic, refetch, handleToggleAbstract }) => {
  ...
  const updateAbstract = () => {
    const input = { id: abstract.id, text: formText };
  ...
  const handleSaveAndCloseAbstract = () => {
  ...
  return (
    <>
      <div className="flex flex-col text-white">
        <h1 className="self-center mb-5">(Beta Feature)</h1>
        <div className="flex flex-col" ref={clickRef}>
          {formText ? (
            isLoading ? (
              <Loader />
            ) : (
          ...
          {formText && !isLoading && (
            <textarea
              ...
              className={
                "my-2 w-full fit-content rounded-lg p-2 outline-none text-white bg-[#4E376A]/75 placeholder-gray-400 border-none text-sm shadow-inner shadow-lg resize-none opacity-70"
              }
            />
          )}
        </div>
